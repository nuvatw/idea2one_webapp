...
import { setCurrentAgenda, type SetCurrentAgendaResult } from "@/lib/actions/agenda";
...
interface CurrentAgendaSwitcherProps {
...
/**
 * Staff component to switch the current active agenda stage.
 * A1: Only Lily/Asa can switch (controlled by canSwitch prop).
 * A2: Shows confirmation dialog before switching.
 */
export default function CurrentAgendaSwitcher({
  items,
  currentAgendaId,
  canSwitch = false,
}: CurrentAgendaSwitcherProps) {
  const [state, formAction, isPending] = useActionState<SetCurrentAgendaResult | null, FormData>(
    setCurrentAgenda,
    null
  );
  ...
  if (items.length === 0) {
    return (
      <div className="rounded-xl border border-warm-200 bg-warm-50 p-4 shadow-xs">
        <p className="text-sm text-warm-500">尚無 Agenda 資料</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-warm-800">切換目前階段</h3>
        {!canSwitch && (
          <span className="text-xs text-warm-400">僅限上哲 / Lily / Asa 操作</span>
        )}
      </div>

      {state?.error && (
        <div className="rounded-xl bg-danger-50 p-3 text-sm text-danger-600" role="alert">
          {state.error}
        </div>
      )}

      {state?.success && (
        <div className="rounded-xl bg-success-50 p-3 text-sm text-success-600" role="status">
          階段已切換
        </div>
      )}
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              disabled={isPending || isCurrent || !canSwitch}
              ...
              } ${isPending || !canSwitch ? "opacity-50 cursor-not-allowed" : ""}`}
            >
              {isPending ? (
      ...
      {/* Confirmation dialog (A2) */}
      <ConfirmDialog
        open={!!confirmTarget}
        title="切換階段"
        message={`確定要切換到「${confirmTarget?.name ?? ""}」嗎？`}
        confirmLabel="確認切換"
